...
//This code is modified from la_svm.c and la_test.c
...
/* Data and model */
int m=0;                          // size of previous training data. After training m= m+msz
int msz;                           //Size of current training data
int msv;                          // number of support vectors
vector <lasvm_sparsevector_t*> X; // feature vectors of training data
...
vector <double> x_sv_square;         // norms of suppport vectors, used for RBF
...
{
    //@K
    //Use kernel function to compute the distance between test vector and support vector
    //This kernel function is used for classifiying data 
    double dot;
    dot=lasvm_sparsevector_dot_product(X_sv[i], testvector);
    //cout<<"dot: "<<dot<<", ";
    //cout<<"x_sq: "<<x_sq<<endl;
    //cout<<"size of x_sv_square: "<<x_sv_square.size()<<endl;
    //for (int k=0; k<x_sv_square.size(); k++)
        //cout<<x_sv_square[k]<<", ";
    ...
    case RBF:
        //cout<<"exp: "<<exp(-kgamma*(x_sv_square[i] + x_sq -2*dot))<<endl;
        return exp(-kgamma*(x_sv_square[i] + x_sq -2*dot));
    case SIGMOID:
...
{
    //This kernel function is used for training data.
    double dot;
    kcalcs++;
...
//The difference from original version is the input format.
{  
    //@K
    int index; double value;
    lasvm_sparsevector_t* v;
    //int msz = trainingdata.size(); 
    ...
    for (std::vector<LabelData>::const_iterator it = trainingdata.begin(); it != trainingdata.end(); ++it)
    {
        ...
        v=lasvm_sparsevector_create();
        for (VSMVector::const_iterator vsmit = it->data.begin(); vsmit != it->data.end(); ++vsmit)
        {
            index = vsmit->feature;
            value = vsmit->value;
...
void load_data_file()
{
    ...
    case 0: // libsvm format
        msz=libsvm_load_data(input_file_name); break;
    case 1: 
        msz=binary_load_data(input_file_name); break;
    case 2:
        ft=split_file_load(input_file_name);
        if(ft==0)      
        {msz=libsvm_load_data(input_file_name); break;} 
        else
        {msz=binary_load_data(input_file_name); break;}
...
void train_online()
{
    ...
    cout<<"[train_online] m: "<<m<<endl;//Before load_data, m is equal to size of previous training data
                                        //After load_data, m is equal to size of all training data
    cout<<"[train_online] msz: "<<msz<<endl;
    for(i=base;i<m;i++) inew.push_back(i); //@K: add new training examples
    
    cout<<"[train_online] inew size: "<<inew.size()<<endl;
    cout<<"[train_online] iold size: "<<iold.size()<<endl;
    cout<<"[train_online] select_size's size: "<<select_size.size()<<endl;
    
    // first add 5 examples of each class, just to balance the initial set
    int c1=0,c2=0;
    if(m==msz)
    {
    // @K: only perform this step for the first training 
        for(i=0;i<msz;i++)
        {
    ...
    cout<<"[train_online] base: "<<base<<endl;
    for(j=0;j<epochs;j++)
    {
        for(i=base;i<m;i++)
        {   //cout<<"i: "<<i<<" inew.size(): "<<inew.size()<<endl;
            ...
            { 
                ...
                {  
				    cout<<"[train_online] inside termination condition"<<endl;
        ...
        for(i=base;i<m;i++) inew.push_back(i);
    }
    inew.resize(0); //reset for next training
    cout<<"\nDone training"<<endl;
    if(saves<2) 
    {
    ...
    for(int j=0;j<2;j++)
        ...
        for(int i=0;i<m;i++)
        {
            ...
            alpha_sv.push_back(alpha[i]);
            X_sv.push_back(X[i]);
            if(kernel_type==RBF)
                x_sv_square.push_back(lasvm_sparsevector_dot_product(X[i],X[i]));
        }
